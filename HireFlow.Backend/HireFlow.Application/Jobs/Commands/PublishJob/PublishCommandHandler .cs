...
{
    public class PublishCommandHandler : IRequestHandler<PublishJobCommand, Result>
    {
        private readonly IJobRepository _jobRepository;
        private readonly IUnitOfWork _unitOfWork;
        private readonly ICurrentUserService _currentUserService;
        public PublishCommandHandler(IJobRepository jobRepository, IUnitOfWork unitOfWork, ICurrentUserService currentUserService)
        {
            _jobRepository = jobRepository;
            _unitOfWork = unitOfWork;
            _currentUserService = currentUserService;
        }
        public async Task<Result> Handle(PublishJobCommand request, CancellationToken ct)
        {
            ...
            if (job == null)
                return Result.Fail($"Cannot publish job: Job with Id '{request.Id}' does not exist.");
